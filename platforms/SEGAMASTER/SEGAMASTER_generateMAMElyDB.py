...
def generate_segamaster_db(platform_dir=None):
    ...
    print(f"Generating Sega Master System database at {db_path}...")
    ...
    if os.path.exists(romlist_path):
        with open(romlist_path, "r", encoding="utf-8", errors="ignore") as f_in:
            for textline in f_in:
                ...
                raw_title = textline
                if matched_ext:
                    idx = textline.lower().rfind(matched_ext)
                    if idx >= 0:
    ...
    conn.close()
    print(f"Successfully compiled {total_count} Sega Master System games into {db_path}!\n")
