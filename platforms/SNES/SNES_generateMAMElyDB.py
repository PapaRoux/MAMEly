...
def generate_snes_db(platform_dir=None):
    ...
    print(f"Generating SNES database at {db_path}...")
    ...
    if os.path.exists(romlist_path):
        with open(romlist_path, "r", encoding="utf-8", errors="ignore") as f_in:
            for textline in f_in:
                ...
                if not textline:
                    continue
                if rom_ext in textline:
                    raw_title = textline.replace(rom_ext, "").replace("&", "and").replace("*", "-").strip()
                    rom_name = raw_title + rom_ext
    ...
    conn.close()
    print(f"Successfully compiled {total_count} SNES games into {db_path}!\n")
